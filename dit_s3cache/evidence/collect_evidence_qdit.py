"""Collect Stage 0 S3-Cache evidence for Q-DiT W8A8 on ImageNet 256.

This script samples in latent space only. It does not load VAE or decode
images; evidence target is each QuantDiTBlock residual ``r_msa + r_mlp`` along
...
from dit_s3cache.evidence.hooks_qdit import install_qdit_block_hooks, restore_hooks
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("dit_s3cache/outputs/evidence_qdit_w8a8_ddim250.npz"),
    ...
    parser.add_argument(
        "--sanity-output",
        type=Path,
        default=Path("dit_s3cache/outputs/sanity_latents_qdit.pt"),
    )
...
def load_qdit_model(args: argparse.Namespace, device: str) -> tuple[torch.nn.Module, Any]:
    _prepend_qdit_root(args.qdit_root)
    from diffusion import create_diffusion  # type: ignore
    from models.models import DiT_models  # type: ignore
    from qdit.modelutils import add_act_quant_wrapper  # type: ignore
    ...
    model = add_act_quant_wrapper(model, device=device, args=qargs, scales=scales)
...
def main(args: argparse.Namespace) -> None:
    ...
    model, diffusion = load_qdit_model(args, device=device)
    ...
    hooks = install_qdit_block_hooks(model, storage)
...
def build_metadata(
    ...
    meta = {
        "stage": "S3-Cache Stage 0 Evidence Collection (Q-DiT)",
        "format": "dit_s3cache_qdit_v1",
        "model": args.model,
