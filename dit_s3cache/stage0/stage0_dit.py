...
def save_object_npy_numpy1_compat(path: Path, arr: np.ndarray) -> None:
    """
    Save an object .npy that NumPy 1.x can read even when generated by NumPy 2.x.

    NumPy 2 pickles object arrays with ``numpy._core`` module paths. NumPy 1.x
    does not provide that module, so Stage 1 loaders running in older envs fail
    before they can validate the payload.
    """
    arr = np.asanyarray(arr)
    if arr.dtype != object:
        raise TypeError(f"Expected object dtype, got {arr.dtype}")

    buf = io.BytesIO()
    pickle.dump(arr, buf, protocol=3)
    payload = buf.getvalue()
    payload = payload.replace(b"cnumpy._core.multiarray\n", b"cnumpy.core.multiarray\n")
    payload = payload.replace(b"cnumpy._core\n", b"cnumpy.core\n")

    with open(path, "wb") as f:
        npy_format.write_array_header_1_0(
            f,
            {
                "descr": npy_format.dtype_to_descr(arr.dtype),
                "fortran_order": False,
                "shape": arr.shape,
            },
        )
